...
a = [
map(int, "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08".split(" ")), # 0
map(int, "49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00".split(" ")), # 1
map(int, "81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65".split(" ")), # 2
map(int, "52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91".split(" ")), # 3
...
def main():
    ...
    for i in range(20):
        for j in range(20):
            ...
            # Up
            if j >= 3:
                b.append(a[i][j]*a[i][j-1]*a[i][j-2]*a[i][j-3])
                # UpLeft
                if i >= 3:
                    b.append(a[i][j]*a[i-1][j-1]*a[i-2][j-2]*a[i-3][j-3])
                # UpRight
                if i <= 16:
                    b.append(a[i][j]*a[i+1][j-1]*a[i+2][j-2]*a[i+3][j-3])
            ...
            # Left
            if i >= 3:
                b.append(a[i][j]*a[i-1][j]*a[i-2][j]*a[i-3][j])
